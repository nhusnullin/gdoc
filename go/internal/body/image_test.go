...
// A GIF is refused: the house style carries PNG and JPEG, and a format nothing
// downstream reads is better named here than dropped by Word.
func TestAGIFIsRefusedByName(t *testing.T) {
	gif := append([]byte("GIF89a"), 0x40, 0x00, 0x40, 0x00)
	if _, err := identify(gif); err == nil {
		t.Fatal("a GIF was accepted, and only PNG and JPEG are read")
	}
}
...
func TestAPictureThatIsNotThereNamesTheLineAndTheFile(t *testing.T) {
	...
	for _, want := range []string{"line 12", "missing.png", "testdata/docs"} {
