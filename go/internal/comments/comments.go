...
// the Docs read placed none or placed one docs.Document.Places refuses: the
// thread still comes back, with its quoted text, and the command puts the ids
// in warnings. A missing anchor is a fact about one thread, never a reason to
// fail the read of all of them.
//
...
func Threads(raw []RawComment, d *docs.Document) ([]Thread, []string) {
	...
	for _, c := range raw {
		...
		if r, ok := ranges[c.ID]; ok && d.Places(r) {
			// A copy, so the caller cannot reach into the document's map.
			t.Range = &r
		} else {
			// docs.Document.Places is the same rule internal/view marks from, so
			// the two commands cannot disagree about what counts as a position.
			...
			// than documented, so an empty pair, an inverted pair, an index
			// outside the text and a tab the document does not have are all
			// shapes it can hand over. This is the command whose output names
			// the range as a position, so reporting one gdoc would refuse to
			// mark would be a false fact in the field a proposal is placed from.
