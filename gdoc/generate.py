...
def generate(
    ...
    *,
    ...
    title overrides the note's own title for this run only. Nothing is written
    back into the note, so an approved title can be used before anyone edits it.
    The plain pandoc path has no cover to put it on, so it ignores it.
    """
    if not md_path.exists():
        raise FileNotFoundError(f"markdown file not found: {md_path}")
    if template is not None:
        return _through_template(drive, md_path, name, out_path, folder_id, template, title)
...
def _through_template(drive, md_path, name, out_path, folder_id, template, title) -> Result:
    """Render through the house template, with the page numbers Google measured."""
    out_path.parent.mkdir(parents=True, exist_ok=True)
    if not folder_id:
        render.build(md_path, out_path, template=template, title=title)
        return Result(docx_path=out_path, reason=_no_folder(out_path, blank_pages=True))

    with tempfile.TemporaryDirectory() as workspace:
        scratch = Path(workspace)
        blank = render.build(
            md_path, scratch / "blank.docx", template=template, title=title
        ).docx_path
        ...
        try:
            ...
            render.build(md_path, out_path, template=template, title=title, pages=pages)
            created = upload_as_gdoc(drive, out_path, name, folder_id)
        except HttpError as error:
            return _upload_refused(
                drive, error, folder_id, md_path, out_path, template, title, measured
            )
        except Exception:
