...
| `unverified` | published before the tool recorded which document a snapshot came from | use them, and say they are unverified |
...
`missing` and `stale` are not failures and they do not stop the run. Say which
one it is, then carry on to the items.

`suggestions_error` naming `documents.readonly` means the token predates this
feature. Tell Nail to run `$GDOC auth login` once, and say that suggestions were
not read on this run.
