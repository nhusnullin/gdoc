...
func firstBlockRuns(t *testing.T, source string) []Run {
	t.Helper()
	bytes := []byte(source)
	root := markdown().Parser().Parse(text.NewReader(bytes))
...
func find(t *testing.T, runs []Run, text string) Run {
	t.Helper()
	for _, run := range runs {
...
func TestMarksAreCarriedOntoTheRunsTheyCover(t *testing.T) {
	...
	if got := find(t, runs, "bold"); !got.Bold || got.Italic {
		t.Errorf("bold run = %#v", got)
	}
	if got := find(t, runs, "italic"); !got.Italic || got.Bold {
		t.Errorf("italic run = %#v", got)
	}
	if got := find(t, runs, "code"); !got.Mono {
		t.Errorf("code run = %#v", got)
	}
	if got := find(t, runs, "marked"); got.Highlight != "yellow" {
...
func TestStrikethroughKeepsItsText(t *testing.T) {
